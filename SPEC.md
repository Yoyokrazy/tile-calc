...
### Cut Edge Physics
Every physical tile starts with 4 puzzle-tab edges. Each cut creates 2 flat edges (one on each piece). Rules:
...
Cut edges are oriented to face WALLS, not other tiles. The simulation uses
`computeCutEdges()` which checks the room bitmap at each tile position:
- If a side borders a wall (no room pixel outside) → that side gets the cut edge
- If a side borders another tile → that side keeps its tab edge
- This ensures puzzle-tab edges always mesh with adjacent tiles
- Cut (flat) edges only face room boundaries where endcap strips cover them
